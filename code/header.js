const light = `<svg width="16" height="16" viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg">
<path fill="currentColor" d="M120 40V16a8 8 0 0 1 16 0v24a8 8 0 0 1-16 0Zm72 88a64 64 0 1 1-64-64a64.07 64.07 0 0 1 64 64Zm-16 0a48 48 0 1 0-48 48a48.05 48.05 0 0 0 48-48ZM58.34 69.66a8 8 0 0 0 11.32-11.32l-16-16a8 8 0 0 0-11.32 11.32Zm0 116.68l-16 16a8 8 0 0 0 11.32 11.32l16-16a8 8 0 0 0-11.32-11.32ZM192 72a8 8 0 0 0 5.66-2.34l16-16a8 8 0 0 0-11.32-11.32l-16 16A8 8 0 0 0 192 72Zm5.66 114.34a8 8 0 0 0-11.32 11.32l16 16a8 8 0 0 0 11.32-11.32ZM48 128a8 8 0 0 0-8-8H16a8 8 0 0 0 0 16h24a8 8 0 0 0 8-8Zm80 80a8 8 0 0 0-8 8v24a8 8 0 0 0 16 0v-24a8 8 0 0 0-8-8Zm112-88h-24a8 8 0 0 0 0 16h24a8 8 0 0 0 0-16Z"/></svg>`;

const moon = `<svg width="16" height="16" viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg">
<path fill="currentColor" d="M233.54 142.23a8 8 0 0 0-8-2a88.08 88.08 0 0 1-109.8-109.8a8 8 0 0 0-10-10a104.84 104.84 0 0 0-52.91 37A104 104 0 0 0 136 224a103.09 103.09 0 0 0 62.52-20.88a104.84 104.84 0 0 0 37-52.91a8 8 0 0 0-1.98-7.98Zm-44.64 48.11A88 88 0 0 1 65.66 67.11a89 89 0 0 1 31.4-26A106 106 0 0 0 96 56a104.11 104.11 0 0 0 104 104a106 106 0 0 0 14.92-1.06a89 89 0 0 1-26.02 31.4Z"/>
</svg>`;

const debbuger = document.getElementById('debugger')

const header = document.getElementById('header')
const theme = document.body.classList;
const btnTheme = document.getElementById("btn-theme");
const btnToSky = document.getElementById("tosky");

const handleStatusTheme = () => {
  return document.body.classList.contains("light")
}

const themeFunc = () => {
  handleStatusTheme()
    ? (btnTheme.innerHTML = light)
    : (btnTheme.innerHTML = moon)
}
  
const handlerTheme = ()=> {
    handleStatusTheme()
    ? theme.replace("light", "dark")
    : theme.replace("dark", "light")
    
    themeFunc()
}
  
// const toSkyBtnStatus = () => {
//   scrollY > 0
//     ? btnToSky.style.right = "50px"
//     : btnToSky.style.right = "-60px"
// }

// const toSky = () => {
//   scrollTo(0, 0);
// }

addEventListener("load", themeFunc);
// addEventListener("scroll", toSkyBtnStatus);
btnTheme.addEventListener("click", handlerTheme);
// btnToSky.addEventListener("click", toSky)

const hiddenElements = document.querySelectorAll('.hidden')

const handleIntersection = (entries) => {
  entries.forEach(entry => {
    entry.isIntersecting && (
      entry.target.classList.add('show'),
      observer.unobserve(entry.target)
    )
  })
}

const observer = new IntersectionObserver(handleIntersection, {threshold: 0.25})
hiddenElements.forEach(element => observer.observe(element))

// const handleNavbarVisibility = (e) => {
//   switch (e.type) {
//     case('mouseover'):
//       // scrollY > 0
//       //   ? header.addEventListener('mouseout', handleNavbarVisibility)
//       //   : header.removeEventListener('mouseout', handleNavbarVisibility)

//       header.style.top = '0px'
//       break;
    
//     case('mouseout'):
//       scrollY != 0 &&
//       setTimeout(() => {
//       header.style.top = '-70px'
//       }, 5000)
//       break;

//     case('scroll'):
//       debbuger.innerHTML = 'scrollY position: ' + scrollY
//       const debbugerPosition = scrollY
    
//       scrollY > 0
//         ? (
//           header.style.top = '-70px'
//         )
//         : (
//           header.style.top = '0px'
//         )
//       break;
//   }

// }

// addEventListener('scroll', handleNavbarVisibility)
// header.addEventListener('mouseover', handleNavbarVisibility)

document.querySelectorAll("nav a").forEach(enlace => {
  enlace.addEventListener("click", function(event) {
      event.preventDefault(); // Evita el comportamiento predeterminado del enlace
      
      const destino = document.querySelector(this.getAttribute("href")); // Obtiene el elemento de destino
      const offset = destino.offsetTop; // Obtiene la posición vertical del elemento
      
      // Realiza el desplazamiento suave
      if (innerWidth >= 500) {
        window.scrollTo({
            top: offset - 120,
            behavior: "smooth"
        });
      } else {
        window.scrollTo({
          top: offset - 40,
          behavior: "smooth"
        });
      }
  });
});

const menu = document.getElementById('menu')

menu.addEventListener('click', () => {
  header.classList.toggle('hide')

  header.classList.contains('hide')
    ? (
      menu.classList = 'icon-menu',
      header.style.right = `-${header.clientWidth}px`
      )
    : (
      menu.classList = 'icon-cross',
      header.style.right = '0'
    )
})


const handleViewPort = () => {
  innerWidth <= 500
  ? (
    header.style.right = `-${header.clientWidth}px`,
    header.classList.add('hide'),
    menu.style.display = 'inherit'
    )
  : (
    menu.style.display = 'none',
    header.classList.remove('hide'),
    header.style.right = '0'
  )
}

addEventListener('resize', handleViewPort)
addEventListener('load', handleViewPort)